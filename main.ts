...
type Operation = "divide" | "multiply" | "subtract" | "add";

let firstOperand: string = "";
let secondOperand: string = "";
let currentOperand: "first" | "second" = "first";
let operation: Operation | null = null;

function buildOperand(numFromButton: string | undefined) {
	if (!numFromButton) return;
...
function selectOperation(operationFromButton: Operation) {
...
numberButtons.forEach((numberButton) => {
	if (numberButton instanceof HTMLElement) {
		numberButton.addEventListener("click", () =>
			buildOperand(numberButton.dataset["number"])
		);
	}
});
...
operationButtons.forEach((operationButton) => {
	if (operationButton instanceof HTMLElement) {
		operationButton.addEventListener("click", () =>
			selectOperation(operationButton.dataset["operation"] as Operation)
		);
	}
});
